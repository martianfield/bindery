...
const files_in_directory = (account_name, repo_slug, revision, directory, files) => {
  if(files === undefined) {
    files = []
  }
    ...
    get(path)
      .then(data => {
        ...
        for(let i=0; i < data.files.length; i++) {
          files.push(data.files[i].path)
        }
        // are there subdirectories?
        if(data.directories.length > 0) {
          let ps = []
          for(let i=0; i<data.directories.length; i++) {
            ps.push(files_in_directory(account_name, repo_slug, revision, data.directories[i], files))
          }
