  ...
  return new Promise((resolve, reject) => {
    ...
    changesets(account_name, repo_slug, 1)
      .then(data => {
        ...
        return files_in_revision(account_name, repo_slug, revision)
      })
      .then(data => {
        resolve(data)
      })
...
const files_in_revision = (account_name, repo_slug, revision) => {
  let path = `https://api.bitbucket.org/1.0/repositories/${account_name}/${repo_slug}/src/${revision}/`
  return get(path)
}
